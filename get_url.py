...
def get_urls_ktrk_ru(soup):
    links = list()
    for link in soup.select('div.section.search-news-section a.post-title')[:5]:
        links.append(link.get('href'))
    return links
...
def get_urls_akipress_org(soup):
    ...
    for link in soup.select('a.aki-news-link'):
        links.append('http://akipress.org' + link.get('href'))
    return links
...
def get_urls_media_center_kg(soup):
    links = list()
    for link in soup.select('div.search-result-list.dash h2 > a')[:5]:
